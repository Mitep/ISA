window.onload = function(){
	...
	$.ajax({
				url: "user/getListaRezervacija" ,
				type:"GET",
				contentType:"application/json",
				dataType:"json",
				success : function(data){
					 $(".listaRezervacija").empty();
					 for(i=0;i<data.length;i++){
						 $(".listaRezervacija").append("<tr><td>" + data[i].projection.name + "</td><td>" + data[i].seat.rowNum+ "</td><td>" + data[i].seat.colNum + "</td></tr>");
						 }
...
function sortTable4(n) {
	  var table, rows, switching, i, x, y, shouldSwitch, dir, switchcount = 0;
	  table = document.getElementById("tableForReservation");
	  switching = true;
	  //Set the sorting direction to ascending:
	  dir = "asc"; 
	  /*Make a loop that will continue until
